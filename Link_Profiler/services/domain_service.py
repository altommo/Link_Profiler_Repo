...
class AbstractDomainAPIClient(BaseAPIClient):
    ...
    async def validate_domain(self, domain_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            response = await self.resilience_manager.execute_with_resilience( # Use resilience_manager
                lambda: self._make_request("GET", self.base_url, params=params),
    ...
    async def whois_lookup(self, domain_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            response = await self.resilience_manager.execute_with_resilience( # Use resilience_manager
                lambda: self._make_request("GET", self.whois_base_url, params=params),
                url=self.whois_base_url # Use whois_base_url for circuit breaker naming
            )
            return await response.json()
        except Exception as e:
...
class WhoisJsonAPIClient(BaseAPIClient):
    ...
    async def whois_lookup(self, domain_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            response = await self.resilience_manager.execute_with_resilience( # Use resilience_manager
                lambda: self._make_request("GET", url, headers=headers),
