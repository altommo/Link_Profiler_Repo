...
async def submit_crawl_to_queue(request: StartCrawlRequest) -> Dict[str, str]: # Use StartCrawlRequest for request type hint
    ...
    # Ensure job_type is set in the config, defaulting if not provided.
    config_data = request.config if request.config is not None else {}
    if "job_type" not in config_data:
        config_data["job_type"] = "backlink_discovery" # Default job type if not specified in config
    ...
    job = CrawlJob(
        ...
        config=crawl_config, # Pass CrawlConfig Pydantic model
        initial_seed_urls=request.initial_seed_urls, # Pass initial_seed_urls
