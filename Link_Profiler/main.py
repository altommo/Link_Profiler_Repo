...
from Link_Profiler.api.schemas import (
    ...
    TopicClusteringRequest, Token, UserCreate, UserResponse # Also import auth/user models
)
...
app.include_router(auth_router) # Prefix is defined in auth_router itself
app.include_router(users_router) # Prefix is defined in users_router itself
...
async def get_content_gap_analysis_result(target_url: str, current_user: Annotated[User, Depends(get_current_user)]): # Protected endpoint
    """
...
async def perform_topic_clustering_endpoint(request: TopicClusteringRequest, current_user: Annotated[User, Depends(get_current_user)]): # Protected endpoint
    """
